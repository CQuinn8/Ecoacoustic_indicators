...
# customize layout
# temp_data <- temp_df_remod %>%
#   add_partial_residuals(mod2) %>%
#   select('s(Anthropophony)', 's(Biophony)', 's(Geophony)', 's(Quiet)',
#          Anthropophony, Biophony, Geophony, Quiet) %>%
#   gather(var, value, -ACI_0)
...
# Read in ACI GAM with Interference included
...
(gg <- slope_df_filtered_combined %>%
    mutate(var = factor(var, levels = c('Anthropophony', 'Biophony','Geophony', 'Quiet', 'Interference'))) %>%
